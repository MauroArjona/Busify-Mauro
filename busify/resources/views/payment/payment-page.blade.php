...
@endsection
@section('content')
    <main class="page payment-page" style="height: 100%;">
        <section class="clean-block payment-form dark" style="height: 100%;">
            <div class="container" style="max-width: 50em;">
                <div class="block-heading" style="padding: 0">
                    <h2 class="text-info">Ingresar pago</h2>
                    <p>Aquí encontrará la información del resumen mensual de la cuota a pagar. Para concretar el pago de
                        la/s cuota/s seleccione "Finalizar pago".</p>
                </div>

                <div class="border rounded-0 border-primary products" style="margin:1em;">
                    <h3 class="title">Resúmen</h3>
                    @foreach ($fees as $fee)
                        <div class="item"><span class="price">{{number_format( $fee->fee_amount, 2)}}</span>
                            <p class="item-name">Cuota de {{ date('m/Y', strtotime($fee->fee_expiration_date)) }}</p>
                        </div>
                    @endforeach
                ...
                <div style="text-align: center;">
                    ...
                    <button class="btn btn-primary" id="btnPayment">Proceder al pago</button>
                </div>
                <div style="display: none" id="loadingAnimation">
                    <div class="lds-ring" style="right: 23em">
                        <div></div>
                        <div></div>
                        <div></div>
                        <div></div>
                ...
                <div style="padding-bottom: 4em;" id="containerPaymentForm" hidden>
                                    ...
                                    <g id="Front">
                                        ...
                                        <g>
                                            ...
                                            <text transform="matrix(1 0 0 1 479.3848 417.0097)"
                                                class="st2 st10 st11">HASTA</text>
                    ...
                    <div class="form-container">
                        ...
                        <div class="field-container">
                            <label for="securitycode">Código seguridad</label>
                            <input id="securitycode" type="text" pattern="[0-9]*" inputmode="numeric">
                        </div>
                        <ul style="color: #ff0000" id="ulErrors">
                        </ul>
                    </div>

                    <form action="{{ route('fee.update', implode(',', $fees->pluck('id')->toArray())) }}"
                        onsubmit="validatePayment(event, this);" method="POST"
                        style="border: 0; width:20em;background: transparent;">
                        @csrf
                        @method('PUT')
                        <button id="btnFinishPayment" class="btn btn-primary d-block w-100" type="submit">Finalizar
                            pago</button>
                    </form>
                </div>
            </div>
    ...
    <script src="{{ asset('js/validationPayment.js') }}"></script>
@endsection
